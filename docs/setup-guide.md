...
| Node.js | 22+ | promptfoo CLI (promptfoo requires Node `^20.20.0 \|\| >=22.22.0` at runtime — see `docs/known-limitations.md`) |
...
## Step 9: Create the ConfigMaps the Job Manifests Reference

Both are required — skip either one and the corresponding Job's pod sits in
`ContainerCreating` forever (`FailedMount ... configmap "X" not found`),
which `kubectl wait`/`make eval-*-wait` won't fail fast on.

```bash
kubectl create configmap golden-dataset \
  --from-file=golden_dataset.json=./data/golden_dataset.json
kubectl create configmap promptfoo-config \
  --from-file=promptfoo/promptfooconfig.yaml
```
...
Prefer the Makefile targets over raw `kubectl apply` — Jobs are immutable, so
re-applying the same manifest after a prior run silently no-ops instead of
rerunning; `make eval-*` deletes the Job first:

```bash
# Quality cycle (RAGAS + promptfoo)
make eval-quality

# Safety cycle (PyRIT XPIA + Garak + Fairlearn)
make eval-safety
```

To exercise a single Job directly instead:
```bash
kubectl delete job ragas-sweep --ignore-not-found
kubectl apply -f k8s/jobs/ragas-sweep.yaml
kubectl logs -f job/ragas-sweep
```

On a persistent (non-Codespace) cluster, `k8s/cronjobs/*.yaml` run the same
Jobs on a schedule via `scripts/run_quality_cycle.py`/`run_safety_cycle.py` —
see `docs/known-limitations.md` for why these must never be `kubectl apply`d
inside a Codespace.
